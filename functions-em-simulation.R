...
### This code is used to produce the simulation tables for our paper.
### The example at the end produces Table 2 in the main paper.
### Further details are given in the READ-ME.pdf file.
...
data.gen= function(pc,seed)
{
  set.seed(seed)

  nsub= pc$nsub*10
  myx= rbinom(nsub,1,pc$trueg1)

  cc= claytonCopula(pc$claytontheta, dim = 2)
  t1t2= rCopula(nsub,cc)
  oldt1= t1t2[,1]
  oldt2= t1t2[,2]

  t1= pc$wei.scale.t1*(-log(1-oldt1)/exp(pc$wei.gamma*myx))^(1/pc$wei.shape.t1)
  t2= pc$wei.scale.t2*(-log(1-oldt2)/exp(pc$wei.gamma*myx))^(1/pc$wei.shape.t2)

  A= rexp(nsub,rate=pc$alambda)  ## rate=lambda, mean=1/lambda

  d= cbind(t1,A,myx)

  truncid= which(t1+t2<A)
  length(truncid)*100/nrow(d)    ## truncation percentage

  d= d[-truncid,]                ## removing subjects who died before the study i.e. truncated
  d[which(d[,1]>d[,2]),1]= NA
  d= d[1:pc$nsub,]
  colnames(d)= c('AGEATDX','AGEATSTUDY','MUTATION')
...
### lower bound for p1
  ...
  myout= list(p0,lbvec0,ubvec0,p1,lbvec1,ubvec1,outtheta,outthetalb,outthetaub)
  names(myout)= c("p0.est","p0.lower","p0.upper",
                  "p1.est","p1.lower","p1.upper",
...
### A simulation example for the proposed EM algorithm
### input list
...
###
### parameters used in data generation for
### Table 2 in the main paper:
###
         nsub=500,                ## sample size
         trueg1= 0.05,            ## genotype prevalence; theta in paper
         wei.scale.t1= 1,         ## Weibull scale for T1
         wei.shape.t1= 1.5,       ## Weibull shape for T1
         wei.scale.t2= 1,         ## Weibull scale for T2
         wei.shape.t2= 2,         ## Weibull shape for T2
         claytontheta= 0.6,       ## parameter for the Clayton copula model
         wei.gamma= 0.8,          ## log-relative risk associated with mutation status (G)
         alambda= 1.2             ## A ~ Exponential(rate=alambda)
         )

input.data= data.gen(pc,seed=1)   ## we use seed = 1,...,100
